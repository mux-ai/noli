...
// Document is a parsed OKF Markdown document. Links contains normalized local
// document IDs (paths relative to the knowledge root, without .md).
...
// Concept is the renderer input. Its ID is a canonical path such as
// "products/x100". Attributes are emitted as arbitrary frontmatter fields.
type Concept struct {
	ID          string
	Type        string
	Title       string
...
// Relation is a resolved relationship between two canonical concept IDs.
